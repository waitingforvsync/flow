...
> [!NOTE]
> This is a shortcut which assumes that if a sprite is present in a row, it occupies the entire character height. In reality, two sprites in a row might be overlapping in x, but not really overlapping because they occupy different ranges in y, for example, the first two and the last two lines. In this case we will pay the price for unnecessary masking, but it simplifies things hugely.
